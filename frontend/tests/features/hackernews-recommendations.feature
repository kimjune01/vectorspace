Feature: Hacker News Recommendations Sync with Current Chat
  ...
  Scenario: HN recommendations appear after conversation is summarized
    ...
    And the HN topics should be semantically related to machine learning using corpus service
    And the topics should be relevant even if they don't contain exact terms
  ...
  Scenario: HN recommendations update when switching between conversations
    ...
    Then I should see a loading state while fetching recommendations
    And the "From Hacker News" section should update to show web development topics
    And the topics may have some overlap if conversations share themes
  ...
  Scenario: HN recommendations handle various conversation topics
    ...
    And clicking on a topic should open the Hacker News article in a new tab
  ...
  Scenario: HN section gracefully handles service unavailability
    ...
    And the error should be logged to console for debugging
  ...
  Scenario: HN topics are clickable and functional
    ...
    Then the Hacker News article should open in a new browser tab
    And I should see the original HN discussion thread
  ...
  Scenario: Multiple HN topics are displayed appropriately
    ...
    And the topics should be ordered by relevance score multiplied by recency score
  ...
  Scenario: HN recommendations work with conversation updates
    ...
    Then the "From Hacker News" section should update automatically
    And the update should use a fade transition
